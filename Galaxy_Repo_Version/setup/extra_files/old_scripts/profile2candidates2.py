...
refbase    = sys.argv[4].upper()
visrange   = int(sys.argv[5])
mincov     = int(sys.argv[6])
mincov3p   = int(sys.argv[7])
minrelmism = float(sys.argv[8])
minarate   = float(sys.argv[9])
mindiv     = int(sys.argv[10])
minrh6     = float(sys.argv[11])
minpos     = int(sys.argv[12])
maxpos     = int(sys.argv[13])
excludepos = []
if len(sys.argv) > 14 and len(sys.argv[14].strip()) != 0:
    excludepos = map(int, sys.argv[14].strip().split(','))

mismcols = ['A', 'G', 'T', 'C']
mismcols.remove(refbase)
    
with open(sys.argv[1]) as fin, open(sys.argv[2], 'w') as fout, open(sys.argv[3], 'w') as fout2:
    ...
    fout2.write('# Reference base: %s\n' % refbase)
    fout2.write('# Visual range: %d\n' % visrange)
    fout2.write('# Minimum coverage: %d\n' % mincov)
    fout2.write('# Minimum coverage (next position): %d\n' % mincov3p)
    fout2.write('# Minimum relative mismatch rate: %f\n' % minrelmism)
    fout2.write('# Minimum arrest rate: %f\n' % minarate)
    fout2.write('# Minimum diversity: %d\n' % mindiv)
    fout2.write('# Minimum RH6 score: %f\n' % minrh6)
    fout2.write('# Position search interval: [%d, %d]\n' % (minpos,maxpos))
    fout2.write('# Excluded positions: [%s]\n' % ', '.join(map(str, excludepos)))
    fout2.write('# \n')
    fout2.write('# reference segment\tposition\tarrest rate\trelative mismatch rate\trh5 (relative mismatches per' +
               'arrest rate)\trh6 (fold change arrest rate)\t%s' % '\t'.join([b + ' mismatches' for b in mismcols]) + 
               '\tprevious base\tnext base\tnext but one base\tdiversity\tcoverage\n')	
    cwindow = []
    currentrefseg = None
    for line in fin:
        ...
        if (c.base == refbase and c.cov >= mincov and cwindow[visrange+1].cov >= mincov3p and c.relmism >= minrelmism
            ...
            fout.write('%05d\t%d\n' % (c.div, c.cov))
	elif (c.base == refbase and minpos <= c.pos <= maxpos and c.pos not in excludepos):
	    fout2.write('%s\t%d\t%.3f\t%.3f\t%.3f\t%.3f\t' % (c.refseg, c.pos, c.arate, c.relmism, c.rh5, c.rh6))
            fout2.write('%s\t' % '\t'.join(['%.3f' % getattr(c, '%smism' % b.lower()) for b in mismcols]))
            fout2.write('%s\t%s\t%s\t' % (cwindow[visrange-1].base, cwindow[visrange+1].base, cwindow[visrange+2].base))
            fout2.write('%05d\t%d\n' % (c.div, c.cov))
        del cwindow[0]
